...
export default function CashPayment() {
  ...
  console.log("🚀 ~ CashPayment ~ formData:", formData.debitAccount)
...
// }, [accountsList, formData.debitAccount]);
  ...
  const checkBillNumberExists = async (billNumber: string) => {
    ...
    try {
      ...
      const response = await fetch("/custom_apis/cash_payment_list.php", {
        method: "POST",
      ...
      if (result.success && result.data) {
        ...
        if (exists) {
          setErrors((prev) => ({ ...prev, billNumber: t("bill_number_exists", "This Bill Number already exists!") }));
        }
        return exists;
      }
      return false;
...
const handleSave = async () => {
  try {
    ...
    if (!monthObj) {
      alert(t("closing_month_not_found", "Closing month not found. Please contact admin."));
      setLoading(false);
      return;
    }
    if (monthObj.CloseStatus !== "Open") {
      alert(t("closed_month_entry_error", "You cannot make entry for a closed month!"));
      setLoading(false);
      return;
    }
    ...
    if (billExists) {
      alert(t("bill_number_duplicate_alert", "This Bill Number already exists! Please use a different bill number."));
      setLoading(false);
      return;
    }
    ...
    const requiredFields = {
      date: t("error_date_required", "Date is required"),
      billNumber: t("error_bill_required", "Bill number is required"),
      Amount: t("error_amount_required", "Bill amount is required"),
      debitAccount: t("error_debit_required", "Debit account is required"),
      creditAccount: t("error_credit_required", "Credit account is required"),
    };
    ...
    // Naya API payload - custom_apis/create_jv
  const payload = {
  api_key: user?.apiKey || "123",
  ...
  details: [
    ...
    const res = await fetch("/custom_apis/create_cash_payment.php", {
      method: "POST",
      headers: {
    ...
    if (!res.ok || !result.success) {
      throw new Error(result.message || "Failed to create Cash Payment");
    }

    alert(t("cash_payment_created", "Cash Payment created successfully! Virtual Code: ") + (result.virtual_code || "N/A"));
    setLoading(false);
    ...
    setLoading(false);
    console.error("Cash Payment Create Error:", error);
    alert(t("error_prefix", "Error: ") + (error.message || t("something_went_wrong", "Something went wrong")));
  }
};
...
// Cash Payment: Debit = All Accounts, Credit = Cash Accounts
const debitOptions = useMemo(() => {
            ...
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-50 to-pink-50 p-4">
              <div className="bg-white rounded-2xl shadow-lg p-6 max-w-5xl w-full">
                <h1 className="text-2xl font-bold text-center mb-1 text-blue-900">
                  {t("cash_payment_entry", "Cash Payment Entry")}
                </h1>
