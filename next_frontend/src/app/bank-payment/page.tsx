...
export default function BankPayment() {
  ...
  console.log("🚀 ~ BankPayment ~ formData:", formData.debitAccount)
...
// }, [accountsList, formData.debitAccount]);
  ...
  const checkBillNumberExists = async (billNumber: string) => {
    ...
    try {
      ...
      const response = await fetch("/custom_apis/bank_payment_list.php", {
        method: "POST",
...
const handleSave = async () => {
  ...
  details: [
    ...
    const res = await fetch("/custom_apis/create_bank_payment.php", {
      method: "POST",
      headers: {
    ...
    if (!res.ok || !result.success) {
      throw new Error(result.message || "Failed to create Bank Payment");
    }

    alert("Bank Payment created successfully! Virtual Code: " + (result.virtual_code || "N/A"));
    setLoading(false);
    ...
    setLoading(false);
    console.error("Bank Payment Create Error:", error);
...
// Bank Payment: Debit = All Accounts, Credit = Bank Accounts
const debitOptions = useMemo(() => {
            ...
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-50 to-pink-50 p-4">
              <div className="bg-white rounded-2xl shadow-lg p-6 max-w-5xl w-full">
                <h1 className="text-2xl font-bold text-center mb-1 text-blue-900">
                  {t("bank_payment_entry", "Bank Payment Entry")}
                </h1>
