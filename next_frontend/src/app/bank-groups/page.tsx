...
import { bankGroupState, fetchBankGroups, removeBankGroup } from "@/Redux/slices/bankgroupSlice/bankgroupSlice";
...
export default function BankGroupPage() {
    ...
    const [deleteBankGroupId, setDeleteSupplierGroupId] = useState<string | null>(null);
    const [isDeleting, setIsDeleting] = useState(false);
    const { bankgrouplist, isLoading, error, total } = useSelector(bankGroupState);

    const deleteBankGroup = async (id) => {
        debugger
        try {
            setIsDeleting(true);
            await deleteData('psbankgroup', id);
            dispatch(removeBankGroup(id));
    ...
    const filteredData = useMemo(() => {
        if (!search) return bankgrouplist;
        return bankgrouplist.filter((row) =>
            Object.values(row).some((value) =>
                String(value).toLowerCase().includes(search.toLowerCase())
            )
        );
    }, [bankgrouplist, search]);
    ...
    // Paginate data
    const bankgrouplistForPagination = useMemo(() => {
    ...
    const handleEdit = (row: SupplierGroupData) => {
        router.push(`/bank-groups/edit/${row.id}`);
    ...
    const confirmDelete = () => {
        deleteBankGroup(deleteBankGroupId)
    };
                        ...
                        <div className="flex justify-between items-center px-6 py-4">
                            <div className="flex items-center">
                                <h1 className="text-2xl font-bold text-gray-900">
                                    {t("page_title_bank_group_list")}
                                </h1>
                            </div>
                            <button
                                onClick={() => router.push("/bank-groups/add")}
                                className="flex items-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition"
                            >
                                <PlusIcon className="w-5 h-5 mr-2" />
                                {t("page_title_add_bank_group")}
                            </button>
                        </div>
                    </header>
                    ...
                    <main className="flex-1 overflow-y-auto">
                        <div className="p-6">
                            ...
                            {isLoading ? (
                                ...
                                <PaginatedTable
                                    columns={columns}
                                    data={bankgrouplistForPagination}
                                    page={page}
                                    pageSize={pageSize}
