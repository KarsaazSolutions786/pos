...
try {
    // Step 1: Insert master without Code to get masterId
		...
		$currentYear = date('y');
		$currentdatetime = now();
		$last_sales_query = DB::query("SELECT * FROM GLTH WHERE (TType = 'Bank Payment') ORDER BY Code DESC LIMIT 1");
    ...
    $gltHData["TBook"] = "Bank Paymen";
    $gltHData["TType"] = "Bank Payment";
    ...
    foreach ($details as $entry) {
        ...
        $gltd["BUnit"] = 1;
        $gltd["TBook"] = "Bank Paymen";
        $gltd["TType"] = "Bank Payment";
